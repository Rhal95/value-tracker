...
function saveToLocalStorage(slot){
    const tableRows = Array.from(document.querySelectorAll("table#valueTable>tbody>tr")
            .entries()
            .map(([_, row])=>{
    ...
    if (slotName && slotName !== '' && tableRows.length > 0){
        ...
        localStorage.setItem(`name_${slot}`, document.getElementById("slotName").placeholder)
    }
    updateSlotNames()
}
...
let activeSlot = Array.from(document.querySelectorAll("menu#saveSlots input")).indexOf(document.querySelector("menu#saveSlots input:checked")) ?? 0;
Array.from(document.querySelectorAll("menu#saveSlots input")).forEach((button, i)=>{
